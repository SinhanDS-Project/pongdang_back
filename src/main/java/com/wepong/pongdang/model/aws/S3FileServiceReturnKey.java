...
@Service
@Slf4j
public class S3FileServiceReturnKey {
	...
	public String uploadFile(MultipartFile file) {
		...
		try (InputStream is = file.getInputStream()) {
			...
			// S3에 파일 업로드
			amazonS3.putObject(amazonS3Config.getBucketName(), key, is, metadata);
		} catch (IOException e) {
    ...
    public void deleteObject(String objectKey) {
        try {
            // S3 버킷에서 객체 삭제 요청
        	amazonS3.deleteObject(amazonS3Config.getBucketName(), objectKey);
